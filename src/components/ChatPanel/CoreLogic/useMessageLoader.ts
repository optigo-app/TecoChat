...
export function useMessageLoader({
  ...
  const loadConversation = useCallback(
    async (page = 1, reset = false, ignoreCache = false, cursorId: number = 0, starOverride?: 0 | 1) => {
      ...
      if (loadingRef.current || !convId || !auth) return;
      ...
      // Use the override if provided (e.g. star toggle), otherwise use the ref
      ...
      if (reset && !ignoreCache) {
        const cached = getConversationFromCache(selectedId);
        if (cached.length > 0) {
      ...
      try {
        // Direction 0 (INITIAL) gives us the latest page of messages
        // directly. No cursor needed — the backend returns the most recent
        // page and sets beforeCursor/afterCursor for pagination.
        ...
        // Safety net: if Direction 0 returned 0 messages and we used a
        // non-zero cursor, retry with cursor 0 to get the latest page.
